...
class AudioDiffusion(nn.Module):
    ...
    def compute_snr(self, timesteps):
        """
        Computes SNR as per:
        github.com/TiankaiHang/Min-SNR-Diffusion-Training/blob/521b624bd70c67cee4bdf49225915f5945a872e3/guided_diffusion/gaussian_diffusion.py#L847-L849
        """
        alphas_cumprod = self.noise_scheduler.alphas_cumprod
        sqrt_alphas_cumprod = alphas_cumprod**0.5
        sqrt_one_minus_alphas_cumprod = (1.0 - alphas_cumprod) ** 0.5

        # Expand the tensors. Adapted from:
        # github.com/TiankaiHang/Min-SNR-Diffusion-Training/blob/521b624bd70c67cee4bdf49225915f5945a872e3/guided_diffusion/gaussian_diffusion.py#L1026
...
class MusicAudioDiffusion(nn.Module):
    def __init__(
        ...
        self.FME = Fundamental_Music_Embedding(
            d_model=d_fme,
            base=base,
            if_trainable=False,
            type=fme_type,
            emb_nn=emb_nn,
            translation_bias_type=translation_bias_type
        )
        self.PE = Music_PositionalEncoding(
            d_model=d_pe,
            if_index=if_index,
            if_global_timing=if_global_timing,
            if_modulo_timing=if_modulo_timing,
            device=self.device
        )
        ...
        self.chord_embedding_layer = Chord_Embedding(
    ...
    def encode_beats(self, beats):
        ...
        # batch, len_beat
        out_beat = torch.tensor(out_beat).cuda()
        out_beat_timing = torch.tensor(out_beat_timing).cuda()
        out_mask = torch.tensor(out_mask).cuda()
    ...
    def encode_chords(self, chords, chords_time):
        ...
        for chord, chord_time in zip(chords, chords_time):  # batch loop
            tokenized_chord_root, tokenized_chord_type, tokenized_chord_inv, tokenized_chord_time, tokenized_chord_mask \
                = self.chord_tokenizer(chord, chord_time)
        ...
        # chords: (B, LEN, 4)
        out_chord_root = torch.tensor(out_chord_root).cuda()
        out_chord_type = torch.tensor(out_chord_type).cuda()
        out_chord_inv = torch.tensor(out_chord_inv).cuda()
        out_chord_timing = torch.tensor(out_chord_timing).cuda()
        out_mask = torch.tensor(out_mask).cuda()
    ...
    def encode_beats_classifier_free(self, beats, num_samples_per_prompt):
        with torch.no_grad():
            ...
            out_beat = torch.tensor(out_beat).cuda()
            out_beat_timing = torch.tensor(out_beat_timing).cuda()
            out_mask = torch.tensor(out_mask).cuda() #batch, len_beat
            embedded_beat = self.beat_embedding_layer(out_beat, out_beat_timing)
    ...
    def encode_chords_classifier_free(self, chords, chords_time, num_samples_per_prompt):
        ...
        with torch.no_grad():
            ...
            out_chord_root_unc = torch.tensor(out_chord_root_unc).cuda()
            out_chord_type_unc = torch.tensor(out_chord_type_unc).cuda()
            out_chord_inv_unc = torch.tensor(out_chord_inv_unc).cuda()
            out_chord_timing_unc = torch.tensor(out_chord_timing_unc).cuda()
            out_mask_unc = torch.tensor(out_mask_unc).cuda()
            embedded_chord_unc = self.chord_embedding_layer(
                out_chord_root_unc, out_chord_type_unc, out_chord_inv_unc, out_chord_timing_unc)
